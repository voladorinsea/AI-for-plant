...
class MenuBar():
    ...
    def my_checkCardClick(self,name):
        for card in self.card_list:
            if(name == plant_name_list[card.name_index]):
                if card.canClick(self.sun_value, self.current_time):
                    return True
                else:
                    return False
    ...
    def my_checkCardFrozen(self):
        result = {}
        for card in self.card_list:
            if card.canClick(10000, self.current_time):
                result[plant_name_list[card.name_index]] = True
            else:
                result[plant_name_list[card.name_index]] = False
        return result
